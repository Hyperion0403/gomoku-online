...
这是一个 18×18 的纯静态五子棋网页，支持本地双人对战，也支持通过 Supabase Realtime 分享房间在线对战。
...
联机双方确认进入同一房间后，都点击“开启语音”并允许浏览器使用麦克风，即可建立点对点语音。

- 支持静音、取消静音和挂断。
- 语音使用 WebRTC，Supabase 只负责交换连接信息。
- Netlify 的 HTTPS 页面可以正常申请麦克风权限。
- 当前使用公共 STUN 服务，部分严格网络环境可能无法建立语音；这种情况需要后续增加 TURN 服务。
